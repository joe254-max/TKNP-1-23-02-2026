...
const StudentClasses: React.FC<Props> = ({ initialTab = 'PHYSICAL', isLecturerPreview = false, onNavigateToProfile }) => {
  ...
  useEffect(() => {
    if (activeView !== 'LIVE_JOIN' || !selectedClassId || !selectedClass) {
      stopTeacherReceiver();
      return;
    }
    ...
    const sessionIsLive =
      liveSessionIsLive &&
      (liveSessionClassId === selectedClassId ||
        Boolean(
          liveSessionTitle &&
            selectedClass.title &&
            normalizeTitle(selectedClass.title).includes(normalizeTitle(liveSessionTitle).slice(0, 6)),
        ));
    ...
    if (liveClassIdRef.current === selectedClassId && (pcRef.current !== null || isConnectingRef.current)) {
      return;
    }

    liveClassIdRef.current = selectedClassId;
    setLiveClassId(selectedClassId);
    isConnectingRef.current = true;
    void connectToLiveClass(selectedClass).finally(() => {
      isConnectingRef.current = false;
    });
  }, [activeView, selectedClassId, liveSessionClassId, liveSessionIsLive, liveSessionTitle]);
          ...
          <div className="rounded-[3rem] bg-slate-950 p-6 shadow-xl relative" ref={feedContainerRef}>
            ...
            {(!teacherStreams.camera && !teacherStreams.screen) && (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white/70">
                <Video size={44} />
                <p className="mt-4 uppercase tracking-[0.35em] text-sm font-black">
                ...
                {teacherFeedStatus === 'CONNECTING' && (
                  <p className="mt-3 max-w-md text-[10px] font-medium normal-case tracking-normal text-white/50">
                    On the lecturer tab: open the same online class and turn the camera ON first. This page will retry every few seconds.
                  </p>
                )}
              </div>
            )}
