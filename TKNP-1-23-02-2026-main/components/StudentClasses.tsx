...
const liveSessionsEqual = (a: LiveSession | null, b: LiveSession | null): boolean => {
  if (a === b) return true;
  if (!a || !b) return !a && !b;
  return (
    a.classId === b.classId &&
...
const syncStudentIdentityFromPortalSession = (): void => {
  const sessionRaw = sessionStorage.getItem('poly_library_user_current');
  const sessionUser = safeParse<Record<string, unknown>>(sessionRaw);
  if (!sessionUser || typeof sessionUser.id !== 'string') return;
  const name = typeof sessionUser.name === 'string' ? sessionUser.name : 'Student';
  localStorage.setItem(CURRENT_STUDENT_ID_KEY, JSON.stringify({ id: sessionUser.id, name }));
};

const getCurrentStudentIdentity = (): { id: string; name: string } => {
  syncStudentIdentityFromPortalSession();
...
const StudentClasses: React.FC<Props> = ({ initialTab = 'PHYSICAL', isLecturerPreview = false, onNavigateToProfile }) => {
  const [activeView, setActiveView] = useState<'LIST' | 'DETAIL' | 'NOT_LIVE' | 'JOIN_LIST' | 'MATERIALS' | 'ASSIGNMENTS' | 'GRADES' | 'SCHEDULE'>('LIST');
  ...
  const loadLiveSession = () => {
    ...
    setLiveSession((current) => (liveSessionsEqual(current, session) ? current : session));
    if (!session) return;
    setMyClasses((prev) => {
      ...
      const changed = next.some((cls, i) => cls.isLive !== prev[i]?.isLive);
  ...
  useEffect(() => {
    ...
    try {
      ...
      channel.onmessage = (event: MessageEvent<LiveSessionBridgePayload>) => {
        ...
        const bridged: LiveSession = {
          classId: payload.classId,
          title: payload.title,
          teacher: payload.teacher,
          isLive: payload.isLive,
        };
        setLiveSession((current) => (liveSessionsEqual(current, bridged) ? current : bridged));
  ...
  const clearTeacherPeerConnection = () => {
    ...
    teacherStreamCountRef.current = 0;
  ...
  const stopTeacherReceiver = () => {
    ...
    liveClassIdRef.current = null;
    setLiveClassId(null);
    clearTeacherPeerConnection();
  ...
  const connectToLiveClass = async (cls: ClassItem) => {
    ...
    const signalingClassId =
      liveSession?.isLive && liveSession.classId ? liveSession.classId : cls.id;
    ...
    pc.onicecandidate = async (event) => {
      ...
      if (teacherIdRef.current) {
        await addSignal(signalingClassId, {
          type: 'candidate',
          classId: signalingClassId,
          from: identity.id,
          role: 'student',
    ...
    const onSignal = async (snapshot: { docChanges: () => Array<{ type: 'added'; doc: { id: string; data: () => Record<string, unknown> } }> }) => {
      for (const change of snapshot.docChanges()) {
        ...
        if (!msg || msg.role !== 'teacher') {
          try {
            if (msg?.role !== 'teacher') await removeSignal(signalingClassId, doc.id);
          } catch {
            // ignore
          }
          continue;
        ...
        try {
          await removeSignal(signalingClassId, doc.id);
        } catch {
          // ignore
        }
      }
    ...
    signalUnsubscribeRef.current = await createSignalListener(signalingClassId, onSignal);

    const sendJoin = () =>
      addSignal(signalingClassId, {
        type: 'join',
        classId: signalingClassId,
        from: identity.id,
        role: 'student',
    ...
    joinRetryTimerRef.current = setInterval(() => {
      if (pcRef.current?.connectionState === 'connected' || teacherStreamsRef.current.camera || teacherStreamsRef.current.screen) {
        stopJoinRetry();
        return;
      }
      void sendJoin();
  ...
  const liveSessionClassId = liveSession?.classId ?? null;
  const liveSessionIsLive = liveSession?.isLive ?? false;
  const liveSessionTitle = liveSession?.title ?? '';
  ...
  const openLiveClassroom = (cls: ClassItem) => {
    const identity = getCurrentStudentIdentity();
    const signalingClassId =
      liveSession?.isLive && liveSession.classId ? liveSession.classId : cls.id;
    const params = new URLSearchParams({
      classId: signalingClassId,
      title: liveSession?.title || cls.title,
      teacher: liveSession?.teacher || cls.teacher,
      className: cls.title,
      studentId: identity.id,
      studentName: identity.name,
    });
    window.location.href = `/student-live-classroom.html?${params.toString()}`;
  };
    ...
    return (
      <div className="px-4 py-8 sm:px-6 lg:px-10">
        ...
        {activeTab === 'ONLINE' && liveSession?.isLive && (
          <div className="mb-8 rounded-[2.5rem] border border-[#e8c8cf] bg-[#fdf6f7] p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-[#3d0413]">Live Right Now</p>
                <h2 className="text-2xl font-black text-[#3d0413] mt-2">{liveSession.title}</h2>
                <p className="text-sm text-[#3d0413]/70">{liveSession.teacher} is live now.</p>
              </div>
              <button
                type="button"
                onClick={() => {
                  ...
                  openLiveClassroom(matchedClass);
                }}
                className="rounded-2xl bg-[#3d0413] px-6 py-4 text-[10px] font-black uppercase tracking-[0.35em] text-white hover:opacity-90 transition"
              >
                Join Live Class
              </button>
            </div>
                    ...
                    <>
                      <div className="flex items-center justify-between">
                        {liveSessionActiveFor(cls) ? (
                          <span className="inline-flex items-center gap-2 rounded-full bg-[#f9e8ec] text-[#3d0413] px-3 py-2 text-[10px] font-black uppercase tracking-[0.35em]">
                      ...
                      <button
                        ...
                        className={`w-full rounded-2xl py-4 text-[10px] font-black uppercase tracking-[0.35em] ${liveSessionActiveFor(cls) ? 'bg-[#3d0413] text-white ring-2 ring-[#e8c8cf]' : 'bg-[#3d0413] text-white'}`}
                      >
                        {liveSessionActiveFor(cls) ? 'Join Live Class' : `Session: ${cls.startTime ?? cls.schedule}`}
                      </button>
                    </>
                  )}
                </div>
