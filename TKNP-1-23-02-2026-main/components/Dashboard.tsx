...
import { addSignal, createSignalListener, removeSignal } from '../lib/tknpSupabaseSignals';
...
import LecturerBroadcastView from './live/LecturerBroadcastView';
import type { LiveParticipant as StreamParticipant, ModAction } from '../types/liveClassroom';
...
const normalizeStudentRecord = (raw: unknown): Student | null => {
  if (!raw || typeof raw !== 'object') return null;
  const row = raw as Record<string, unknown>;
  const name = String(row.name ?? row.studentName ?? row.full_name ?? '').trim();
  const admNo = String(row.admNo ?? row.adm_no ?? row.schoolRegistryId ?? '').trim();
  const id = String(row.id ?? row.student_id ?? `st-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`);
  ...
  return {
    ...
    gradeAverage: Number(row.gradeAverage ?? row.grade_average ?? 0),
    status: (row.status as Student['status']) || 'ACTIVE',
    classId: row.classId ? String(row.classId) : row.class_key ? String(row.class_key) : undefined,
  };
};
...
const StaffDashboardHome: React.FC<DashboardProps> = ({
  ...
  const [students, setStudents] = useState<Student[]>(() => {
    ...
    const joinedStudents: Student[] = saved
          ...
          setStudents((prev) => {
            ...
            studentRows.forEach((r) => {
              const normalized = normalizeStudentRecord({
                ...
                gradeAverage: r.grade_average,
                status: r.status,
                classId: r.class_key,
              });
              if (!normalized) return;
              byAdm.set(normalized.admNo, normalized);
            });
  ...
  const startBroadcast = async (classId: string) => {
    ...
    const ensurePeer = async (studentId: string): Promise<PeerState> => {
      ...
      pc.ontrack = (ev) => {
        ...
        if (!stream) return;
        setStudentRemoteStreams((prev) => ({ ...prev, [studentId]: stream }));
      };
    ...
    const unsub = await createSignalListener(classId, async (snapshot: any) => {
      for (const change of snapshot.docChanges()) {
        ...
        try {
          ...
          if (msg.type === 'join' || msg.type === 'request-offer') {
            const existing = broadcasterPeersRef.current[studentId];
            if (existing && (existing.pc.connectionState === 'failed' || existing.pc.connectionState === 'closed')) {
              try { existing.pc.close(); } catch { /* ignore */ }
              delete broadcasterPeersRef.current[studentId];
            }
            const peer = await ensurePeer(studentId);
            const offer = await peer.pc.createOffer();
            await peer.pc.setLocalDescription(offer);
            await addSignal(classId, { type: 'offer', classId, role: 'teacher', from: user.id, to: studentId, sdp: peer.pc.localDescription ? { type: peer.pc.localDescription.type, sdp: peer.pc.localDescription.sdp } : null });
    ...
    broadcasterSignalRef.current = unsub;
    startRecording();
  };
  ...
  const persistLiveSession = (isLive: boolean) => {
    ...
    try {
      ...
      broadcastLiveSession({
        ...
        classTitle: normalizeTitle(selectedOnlineClass.title),
  ...
  const handleToggleVideo = async () => {
    ...
    if (isCamOn) {
      ...
      try {
        ...
        void startBroadcast(selectedOnlineClass.id);
      } catch (err) {
  ...
  const filteredStudents = useMemo(() => {
    const query = searchText(ledgerSearch);
    return classStudents.filter((s) => {
      if (!query) return true;
      return searchText(s.name).includes(query) || searchText(s.admNo).includes(query);
    });
  ...
  const filteredMaterials = useMemo(() => {
    return lectureNotes.filter(note => {
      ...
      const matchesSearch = query === '' || 
                           searchText(note.title).includes(query) || 
                           searchText(note.fileName).includes(query) ||
                           (note.tags ?? []).some((tag) => searchText(tag).includes(query));
  ...
  const streamParticipants = useMemo((): StreamParticipant[] => {
    return liveParticipants.map((p) => ({
      userId: p.id,
      displayName: p.fullName || p.name,
      role: 'student' as const,
      stream: studentRemoteStreams[p.id] ?? null,
      isMuted: false,
      isSpeaking: false,
      isVideoOn: Boolean(studentRemoteStreams[p.id]?.getVideoTracks().some((t) => t.readyState === 'live')) || p.hasVideo,
      handRaised: Boolean((p as LiveParticipant & { handRaised?: boolean }).handRaised),
      handRaisedAt: (p as LiveParticipant & { handRaisedAt?: number }).handRaisedAt,
    }));
  }, [liveParticipants, studentRemoteStreams]);

  const handleModAction = (action: ModAction, targetUserId: string) => {
    const classId = selectedOnlineClass?.id ?? broadcasterClassIdRef.current;
    if (!classId) return;
    void addSignal(classId, {
      type: 'mod_action',
      classId,
      role: 'teacher',
      from: user.id,
    ...
    const labels: Record<ModAction, string> = {
      mute: 'Mute signal sent',
      lower_hand: 'Hand lowered',
      kick: 'Student removed from session',
      call_on: 'Call-on signal sent',
    };
    showToast(labels[action], 'info');
    if (action === 'kick') {
      setLiveParticipants((prev) => prev.filter((p) => p.id !== targetUserId));
  ...
  const renderOnlineClassDetail = () => (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12 animate-in fade-in slide-in-from-right-12 duration-1000">
      <div className="lg:col-span-2 space-y-8 lg:space-y-12">
        ...
        <video ref={lecturerCameraPiPRef} autoPlay playsInline muted className="sr-only" aria-hidden />
        <LecturerBroadcastView
          lecturerStream={mediaStream}
          ...
          liveStudentCount={selectedOnlineClass?.students ?? liveParticipants.length}
          onModAction={handleModAction}
          controlsSlot={
            <div className="flex flex-wrap items-center justify-center gap-3 px-4 py-3 bg-white/10 backdrop-blur-3xl rounded-2xl border border-white/10 w-full max-w-[480px] mx-auto">
              <button
                type="button"
                onClick={handleToggleMic}
                className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${isMicOn ? 'bg-white text-[#3d0413]' : 'bg-rose-600 text-white'}`}
              >
              ...
              <button
                ...
                className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${isCamOn ? 'bg-white text-[#3d0413]' : 'bg-rose-600 text-white'}`}
              >
              ...
              <button
                ...
                className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${isScreenSharing ? 'bg-emerald-500 text-white' : 'bg-white/10 text-white hover:bg-white/20'}`}
              ...
              {isScreenSharing && (
                <button
                  type="button"
                  onClick={handleToggleScreenShare}
                  className="px-4 py-3 bg-rose-500 hover:bg-rose-600 text-white rounded-2xl text-[9px] font-black uppercase tracking-widest"
                >
                  Stop Share
                </button>
              )}
              <button
                ...
                className="px-6 py-3 bg-rose-600 text-white rounded-2xl text-[9px] font-black uppercase tracking-widest"
              >
                Terminate
              </button>
            </div>
